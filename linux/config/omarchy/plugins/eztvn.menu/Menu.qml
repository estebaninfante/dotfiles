...
Item {
  ...
  // Effective library: the host-scoped one when injected, otherwise the
  // local fallback below. The shell can revoke a clone's scoped
  // PluginShellApi while a keepLoaded panel still references it, which
  // leaves shell.appLibrary null and the Apps menu permanently empty.
  function appLib() {
    return root.appLibrary ? root.appLibrary : fallbackAppLib
  }

  // Local fallback with the same contract as the shell's AppLibrary:
  // DesktopEntries read directly, same hidden-entry filters, same icon
  // lookup, same gtk-launch invocation (without the launch OSD).
  QtObject {
    id: fallbackAppLib
    ...
    function isHiddenEntry(entry) {
      ...
      return fallbackAppLib.configuredHiddenEntryIds[id] === true
        || fallbackAppLib.desktopHiddenEntryIds[id] === true
    }
    ...
    function sortedEntries(query) {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        if (fallbackAppLib.isHiddenEntry(entry)) continue
        var name = fallbackAppLib.entryName(entry)
        if (!name) continue
    ...
    function remove(desktopId, name) {
      var id = String(desktopId || "")
      if (!id) return
      Util.execDetached(Util.shellQuote(fallbackAppLib.omarchyPath + "/bin/omarchy-remove-launcher-entry") + " " + Util.shellQuote(id) + " " + Util.shellQuote(String(name || id)))
    }
    ...
    function loadConfiguredHides(rawText) {
      ...
      for (var i = 0; i < lines.length; i++) {
        var id = fallbackAppLib.normalizeDesktopId(lines[i])
        if (id.length > 0) next[id] = true
      }
      fallbackAppLib.configuredHiddenEntryIds = next
      fallbackAppLib.appsChanged()
    }
    ...
    function loadDesktopHiddenEntries(rawText) {
      ...
      fallbackAppLib.desktopHiddenEntryIds = next
      fallbackAppLib.appsChanged()
    }
    ...
    function hiddenEntryScanCommand() {
      ...
      var script = fallbackAppLib.omarchyPath + "/shell/services/hidden-entries.sh"
      return Util.shellQuote(script) + " " + Util.shellQuote(desktop)
    }

    Component.onCompleted: fallbackHiddenScan.running = true
  }

  FileView {
    id: fallbackHidesFile
    path: fallbackAppLib.omarchyPath + "/default/omarchy/launcher.hides"
    watchChanges: true
    printErrors: false
    onLoaded: fallbackAppLib.loadConfiguredHides(text())
    onFileChanged: fallbackAppLib.loadConfiguredHides(text())
    onLoadFailed: fallbackAppLib.loadConfiguredHides("")
  }

  Process {
    id: fallbackHiddenScan
    command: ["bash", "-c", fallbackAppLib.hiddenEntryScanCommand()]
    stdout: SplitParser { onRead: function(line) { fallbackHiddenOutput.text += line + "\n" } }
    onStarted: fallbackHiddenOutput.text = ""
    onExited: fallbackAppLib.loadDesktopHiddenEntries(fallbackHiddenOutput.text)
  }

  QtObject {
    id: fallbackHiddenOutput
    property string text: ""
  }

  Connections {
    target: DesktopEntries.applications
    function onValuesChanged() { fallbackAppLib.appsChanged() }
  }
  ...
  function mergeAppRows() {
    ...
    // The host can revoke the scoped app library (or DesktopEntries may not
    // be ready yet). Leave the provider unloaded so the next open retries
    // instead of caching an empty Apps menu.
    if (rows.length === 0) {
      var nextLoaded = ({})
      for (var k in root.providersLoaded) if (k !== "apps") nextLoaded[k] = root.providersLoaded[k]
      root.providersLoaded = nextLoaded
      return
    }
    var appRows = []
    for (var j = 0; j < rows.length; j++) {
      ...
      if (!appId) continue
      var subtext = lib.entrySubtext(entry)
  ...
  function activateIndex(index, fromPointer) {
    ...
    if (row.kind === "menu" || row.kind === "link") {
      ...
      opened = false
      filterText = ""
      var pickLib = root.appLib()
      if (pickLib) pickLib.launch(appId, label)
    } else {
  ...
  function confirmDelete() {
    ...
    if (!target) return
    root.cancel()
    var delLib = root.appLib()
    if (delLib) delLib.remove(target.appId, target.label)
  }
  ...
  function openExistingMenu(initialMenu) {
    ...
    var openLib = root.appLib()
    if (openLib) openLib.refreshIcons()
  ...
  PanelWindow {
    id: panel
    property real fade: root.opened && root.rowsLoaded ? 1 : 0
    Behavior on fade {
      NumberAnimation { duration: 140; easing.type: Easing.OutCubic }
    }
    visible: root.rowsLoaded && (root.opened || fade > 0)
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              Image {
                ...
                source: row.isApp ? root.appLib().iconSource(row.appIcon) : ""
                asynchronous: true
                anchors.left: parent.left
