...
Panel {
  ...
  // "fast" | "standard" | "" (desconocido — helper root ausente o sudo sin NOPASSWD)
  property string fastChargeState: ""
  ...
  function refresh() {
    ...
    if (!fastChargeProc.running) fastChargeProc.running = true
  }

  function toggleFastCharge() {
    if (fastChargeAction.running) return
    fastChargeAction.command = ["sudo", "-n", "/usr/local/bin/battery-fastcharge", "toggle"]
    fastChargeAction.running = true
  }
  ...
  // Reaplica el estado de energia tras cambiar de perfil: el guard decide ahorro
  // (bateria O power-saver) vs rendimiento y recarga Hyprland (60/120Hz, blur,
  // anims), dGPU, brillo y syncthing. system guard via sudoers NOPASSWD.
  function reapplyPowerGuard() {
    if (!powerGuardProc.running) {
      powerGuardProc.command = ["sudo", "-n", "systemctl", "restart", "battery-power-guard.service"]
      powerGuardProc.running = true
    }
    if (!sessionGuardProc.running) {
      sessionGuardProc.command = ["systemctl", "--user", "restart", "power-session-guard.service"]
      sessionGuardProc.running = true
    }
  }
  ...
  Process {
    id: fastChargeProc
    command: ["sudo", "-n", "/usr/local/bin/battery-fastcharge", "status"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var v = text.trim()
        root.fastChargeState = (v === "fast" || v === "standard") ? v : ""
      }
    }
    onExited: function(exitCode, exitStatus) {
      if (exitCode !== 0) root.fastChargeState = ""
    }
  }
  ...
  Process {
    id: actionProc
    onExited: {
      root.reapplyPowerGuard()
      root.refresh()
    }
  }

  Process { id: powerGuardProc }
  Process { id: sessionGuardProc }
      ...
      Column {
        ...
        Column {
          ...
          Row {
            ...
            Button {
              width: parent.width
              iconText: root.fastChargeState === "fast" ? "󰂄" : "󰢜"
              iconSize: Style.font.title
              text: root.fastChargeState === "" ? "Fast charge —"
                : (root.fastChargeState === "fast" ? "Fast charge ON" : "Fast charge OFF")
              fontSize: Style.font.bodySmall
              foreground: root.bar.foreground
              fontFamily: root.bar.fontFamily
              horizontalPadding: Style.spacing.controlPaddingX
              verticalPadding: Style.spacing.controlPaddingY + Style.space(2)
              bordered: true
              active: root.fastChargeState === "standard"
              enabled: root.fastChargeState !== ""
              onClicked: root.toggleFastCharge()
            }
          }

          Text {
            width: parent.width
            visible: root.fastChargeState !== ""
            textFormat: Text.PlainText
            text: root.fastChargeState === "fast"
              ? "Charges faster, runs hotter."
              : "Slower charge, less heat."
            color: Qt.darker(root.bar.foreground, 1.4)
            font.family: root.bar.fontFamily
            font.pixelSize: Style.font.caption
            elide: Text.ElideRight
