...
Item {
  ...
  // Neutral gray surface with white text; the wallpaper-derived accent
  // (Color.accent) is reserved for active/highlight elements.
  property color themeForeground: "#ffffff"
  property color themeContrastForeground: "#111111"
  property color transparentForeground: "#ffffff"
  property color foreground: themeForeground
  property color barForeground: useTransparentForeground ? transparentForeground : themeForeground
  property bool foregroundAnimationEnabled: true
  property color background: Qt.rgba(0.12, 0.12, 0.13, 0.90)
  ...
  // Floating islands: each horizontal section paints its own rounded,
  // content-sized island instead of one full-width slab. The window stays
  // taller than the widgets by islandMargin on both sides so the islands
  // hover off the screen edge; that extra height is part of the reserved
  // exclusion zone, so maximized windows still stop clear of the bar.
  property int islandMargin: root.windowGapTop
  // Horizontal inset so each island's outer edge lines up with the outer edge
  // of tiled windows, which sit `general:gaps_out` from the screen edge. The
  // shared Style.gapsOut token is deliberately halved for panels, so we read
  // the raw per-side gaps here to land exactly on the window edge.
  // Vertical gap is pinned: hyprland.lua runs dynamic gaps (8..18 por nº de
  // ventanas) and re-reads them on every workspace switch, which used to
  // resize the bar window and make its height jump. Keep this in sync with
  // GAP_MAX in ~/.config/hypr/hyprland.lua if you change it. Horizontal gaps
  // stay dynamic so the islands keep aligning with the tiled-window edge.
  property int windowGapTop: 18
  property int windowGapRight: Style.gapsOut
  property int windowGapBottom: Style.gapsOut
  property int windowGapLeft: Style.gapsOut
  property int islandPadding: Style.space(8)
  readonly property int islandHeight: root.barSize
  // Square-ish: follow Style.cornerRadius (lee decoration:rounding de Hyprland)
  // con un piso de 2px, en vez del piso pill de 12px.
  readonly property int islandRadius: Math.min(Math.round(root.islandHeight / 2), Math.max(Style.cornerRadius, 2))
  readonly property int barWindowSize: root.windowGapTop + root.islandHeight

  // Read the raw Hyprland gaps_out (Style.gapsOut is intentionally half of it)
  // so the islands can align with the tiled-window edge, not the panel edge.
  function applyWindowGaps(raw) {
    try {
      var parts = String((JSON.parse(raw || "{}")).css || "").match(/-?\d+(?:\.\d+)?/g) || []
      var vals = []
      for (var i = 0; i < parts.length; i++) vals.push(Math.max(0, Math.round(Number(parts[i]))))
      if (vals.length >= 4) {
        windowGapRight = vals[1]
        windowGapBottom = vals[2]
        windowGapLeft = vals[3]
      } else if (vals.length === 1) {
        windowGapRight = vals[0]
        windowGapBottom = vals[0]
        windowGapLeft = vals[0]
      }
    } catch (e) {
      // hyprctl missing / Hyprland not running — keep the previous values.
    }
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.applyWindowGaps(text)
    }
  }

  // Re-read whenever the shared token re-polls Hyprland (theme apply, gaps
  // toggle, config reload) so the islands keep tracking the window edge.
  Connections {
    target: Style
    function onGapsOutChanged() { windowGapsProc.running = true }
  }

  // Gaps are dynamic (they shrink as more windows open), so re-read them on the
  // Hyprland events that change the window count, debounced to one call.
  Connections {
    target: Hyprland
    function onRawEvent(event) {
      if (!event || !event.name) return
      var name = String(event.name)
      if (name === "activewindow") return
      if (name.indexOf("window") !== -1 || name.indexOf("workspace") !== -1 || name === "configreloaded")
        gapsDebounce.restart()
    }
  }
  Timer {
    id: gapsDebounce
    interval: 120
    repeat: false
    onTriggered: windowGapsProc.running = true
  }
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -root.barWindowSize : 0
      bottom: root.barHidden && root.position === "bottom" ? -root.barWindowSize : 0
    ...
    implicitHeight: root.vertical ? 0 : root.barWindowSize
    // Horizontal bars paint transparent and let each island carry the
    // background; vertical bars keep the single slab.
    ...
    Component {
      ...
      Item {
        ...
        // Drag / double-click surface spans the whole window, behind the
        // islands, so empty bar space still moves the bar and toggles
        // transparency while widget clicks stay on top.
        ...
        Rectangle {
          ...
          width: leftContent.width + root.islandPadding * 2
          anchors.left: parent.left
          anchors.leftMargin: root.windowGapLeft
          anchors.top: parent.top
          anchors.topMargin: root.windowGapTop

          // Radius eases alongside the content-driven width morph.
        ...
        Rectangle {
          ...
          border.color: Util.alpha(Color.accent, 0.22)
          height: root.islandHeight
          width: rightContent.width + root.islandPadding * 2
          anchors.right: parent.right
          anchors.rightMargin: root.windowGapRight
          anchors.top: parent.top
          anchors.topMargin: root.windowGapTop
        ...
        Rectangle {
          ...
          border.color: Util.alpha(Color.accent, 0.22)
          height: root.islandHeight
          width: centerContent.implicitWidth + root.islandPadding * 2
          // Pin the anchor widget (not the whole island) to the screen center,
          // so growing/shrinking neighbors (hover-revealed indicators, the
          // clock's weekday) can't drag it sideways. When no anchor is
          // configured this reduces to centering the whole island.
          x: Math.round(parent.width / 2 - root.islandPadding - centerContent.anchorCenterX)
          anchors.top: parent.top
          anchors.topMargin: root.windowGapTop
    ...
    Component {
      ...
      Item {
        ...
        CenterModules { anchors.centerIn: parent }
  ...
  component CenterModules: Item {
    ...
    // Horizontal center of the anchor module within the content row, so the
    // island can be shifted to keep that widget at the screen center. Falls
    // back to the content's own center when there is no anchor (or it hasn't
    // built yet).
    readonly property real anchorCenterX: {
      var row = centerLoader.item
      if (!hasAnchor || !row || !row.anchorModule || row.anchorModule.width <= 0) return implicitWidth / 2
      return row.anchorModule.x + row.anchorModule.width / 2
    }

    // Content-sized so the center island can wrap it. The horizontal
    // arrangement is a Row (vertical a Column) instead of the anchored split
    // the built-in bar uses, because a Row reports its own width and lets the
    // island hug the group. Trade-off: the centerAnchor widget sits at the
    // middle of the group, not the exact middle of the screen.
    ...
    Component {
      ...
      Row {
        ...
        // Exposed so CenterModules can locate the anchor module's center.
        property Item anchorModule: centerAnchorModule
  ...
  component ModuleList: Loader {
    ...
    // Loader.implicitWidth/Height already mirror the loaded item's implicit
    // size; the explicit width/height below make a Positioner (the center
    // Row/Column) measure this list from its real content.
  ...
  component ModuleSlot: Item {
    ...
    width: implicitWidth
    height: implicitHeight
    // Morph each module's footprint so the enclosing island grows/shrinks with
    // its content instead of snapping. The Row/Column implicit size (and thus
    // the island width) follows this same curve, so nothing overflows.
